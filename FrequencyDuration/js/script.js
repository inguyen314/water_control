...
const groupIntervalTextbox = document.getElementById('group-interval-txbox'),
      computeHTMLBtn = document.getElementById('compute-html-btn'),
      computePDFBtn = document.getElementById('compute-pdf-btn'),
      manualBoundCheckbox = document.getElementById('manual-checkbox'),
      autoBoundCheckbox = document.getElementById('auto-checkbox'),
      maxBoundTextbox = document.getElementById('maximum-bound-txbox'),
      minBoundTextbox = document.getElementById('minimum-bound-txbox'),
      ...
      manualValuesDiv = document.querySelector('.bound-control .bound-values'),
      ...
      secundaryTableBody = document.getElementById('secundary-table-body'),
...
function initialize(data) {
    ...
    autoBoundCheckbox.addEventListener('click', autoBoxChecked);
    ...
    groupIntervalTextbox.addEventListener('change', () => {
        if (isEntryDataValid()){
            ...
            if (haveClass(groupIntervalTextbox, 'invalid-data')){
                groupIntervalTextbox.classList.remove('invalid-data')
            }
            ...
            let invalidList = getInvalidTextbox();
            invalidList.forEach((element) => {
                if (!haveClass(element, 'invalid-data')){
            ...
            //     errorMessageText.textContent = "Invalid entry. Verify all red textbox."
            // }
        }
    });

    maxBoundTextbox.addEventListener('change', () => {
        if (isEntryDataValid()){
            computeHTMLBtn.disabled = false;
            computePDFBtn.disabled = false;
        } else {
        ...
        if (isValidNumber(maxBoundTextbox.value)){
            maxBoundTextbox.classList.remove('invalid-data');
        }
    });

    minBoundTextbox.addEventListener('change', () => {
        if (isEntryDataValid()){
            computeHTMLBtn.disabled = false;
            computePDFBtn.disabled = false;
        } else {
        ...
        if (isValidNumber(minBoundTextbox.value)){
            minBoundTextbox.classList.remove('invalid-data');
        }
    });

    singleMonthDayTextbox.addEventListener('change', () => {
        if (isEntryDataValid()){
            computeHTMLBtn.disabled = false;
            computePDFBtn.disabled = false;
        } else {
        ...
        if (isValidNumber(singleMonthDayTextbox.value)){
            singleMonthDayTextbox.classList.remove('invalid-data');
        }
    });

    specificTWFromTextbox.addEventListener('change', () => {

        let iAmValid = false;

        try {
            let testMonth = parseInt(specificTWFromTextbox.value.split('-')[0]);
            let testDay = parseInt(specificTWFromTextbox.value.split('-')[1]);
            iAmValid = (isValidNumber(testMonth) && isValidNumber(testDay)) ? true : false;
        } catch (err) {
            iAmValid = false;
        }
        ...
        if (iAmValid){
            computeHTMLBtn.disabled = false;
            computePDFBtn.disabled = false;
            specificTWFromTextbox.classList.remove('invalid-data');
        } else {
            computeHTMLBtn.disabled = true;
            computePDFBtn.disabled = true;
    ...
    specificTWToTextbox.addEventListener('change', () => {
        if (isEntryDataValid()){
            computeHTMLBtn.disabled = false;
            computePDFBtn.disabled = false;
        } else {
        ...
        if (isValidNumber(specificTWToTextbox.value)){
            specificTWToTextbox.classList.remove('invalid-data');
        }
    });
...
function main(data) {
    ...
    let allStageList = workData.map(item => item.stage);

    // Process data
    let meanvalue = Math.round((allStageList.reduce((acc, cur) => acc + cur, 0) / allStageList.length) * 100) / 100
    let maxValue = Math.round(Math.max(...allStageList) * 100) / 100;
    let minValue = Math.round(Math.min(...allStageList) * 100) / 100;
    ...
    if (manualBoundCheckbox.checked) {
        maxBound = parseInt(Math.round(maxBoundTextbox.value))
    } else if (roundedMax === maxValue) {
        maxBound = roundedMax + 1;
    } else {
        maxBound = roundedMax
    }

    if (manualBoundCheckbox.checked) {
        minBound = parseInt(Math.round(minBoundTextbox.value))
    } else if (roundedMin === minValue) {
        minBound = roundedMin - 1;
    } else {
        minBound = roundedMin
    }
    ...
    let filteredData = noExclusionCheckbox.checked ? allStageList : [];
    ...
    if (exclusionCheckbox.checked && singleMonthCheckbox.checked) {
        ...
        objData.forEach((element) => {
            ...
            if (month === parseInt(singleMonthDayTextbox.value)) {
                tempData.push({
                    date: date,
                    stage: element[1],
                    qualityCode: element[2]
        ...
        consoleLog ? console.log({tempData}) : null;

        filteredData = tempData.map(item => item.stage);
    }
    ...
    if (exclusionCheckbox.checked && singleDayCheckbox.checked) {
        ...
        objData.forEach((element) => {
            ...
            if (day === parseInt(singleMonthDayTextbox.value)) {
                tempData.push({
                    date: date,
                    stage: element[1],
                    qualityCode: element[2]
    ...
    if (exclusionCheckbox.checked && specificTimeWindowCheckbox.checked) {
        ...
        objData.forEach((element) => {
            ...
            } else if( (fromMonth > toMonth) && (month >= fromMonth || month <= toMonth)) {
            ...
            // if ( fromMonth === toMonth && month === fromMonth && day >= fromDay && day <= toDay){
            //     consoleLog ? console.log("From Month === To Month === Month and Day is in between from and to day --> ", {month, fromMonth, toMonth, day, fromDay, toDay, date}) : null;
            //     validDate = true;
            // } else if ( month === fromMonth && day >= fromDay ) {
            //     //consoleLog ? console.log("Month === From Month and Day is >= From day --> ", {month, fromMonth, day, fromDay, date}) : null;
            //     validDate = true;
            // } else if ( month === toMonth && day <= toDay ) {
            //     //consoleLog ? console.log("Month === To Month and Day is <= To day --> ", {month, toMonth, day, toDay, date}) : null;
            //     validDate = true;
            // } else if( month > fromMonth && month < toMonth ) {
            //     //consoleLog ? console.log("Month is in between To Month and From Month --> ", {month, fromMonth, toMonth, date}) : null;
            //     validDate = true;
            // }
    ...
    for (let i = minBound; i <= maxBound; i += groupInterval){
        ...
        let levelPercent = Math.round((levelTotal / filteredData.length) * 10000) / 100;
        ...
        if ((maxBound - i) < groupInterval && (maxBound - i) !== 0){
    ...
    // Stats to show
    let statsMean = noExclusionCheckbox.checked ? meanvalue : Math.round((filteredData.reduce((acc, cur) => acc + cur, 0) / filteredData.length) * 100) / 100;
    let statsMax = noExclusionCheckbox.checked ? maxValue : Math.round(Math.max(...filteredData) * 100) / 100;
    let statsMin = noExclusionCheckbox.checked ? minValue : Math.round(Math.min(...filteredData) * 100) / 100;

    consoleLog ? console.log({mainTableData, secondTableData}) : null;

    consoleLog ? console.log({statsMean, statsMax, statsMin}) : null;

    populateTable(mainTableBody, mainTableData);
    populateTable(secundaryTableBody, secondTableData);

    // Show the statistic numbers
    statsNumbers[0].textContent = statsMean;
    statsNumbers[1].textContent = statsMax;
    statsNumbers[2].textContent = statsMin;
...
function populateTable(tableBody, tableData){
    ...
    for (let i = tableData.length - 1; i >= 0; i--) {
        ...
        newRow.innerHTML = `<td>${tableData[i].level}</td>
                            <td>${tableData[i].total}</td>
                            <td>${tableData[i].percent}</td>`;
...
function isEntryDataValid() {
    ...
    if (manualBoundCheckbox.checked){
        checkList.push({
            name: "Maximum Bound",
            value: maxBoundTextbox.value
        });
        checkList.push({
            name: "Minimum Bound",
            value: minBoundTextbox.value
        });
    }
